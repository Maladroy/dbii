...
import { X, Phone, Check, ZoomIn } from 'lucide-react';
...
const ProductDetailModal: React.FC<ProductDetailModalProps> = ({ product, onClose, onOpenContact }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden relative flex flex-col md:flex-row animate-scale-in">
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 z-20 bg-white/80 hover:bg-white p-2 rounded-full transition-colors shadow-sm"
        >
                ...
                {uniqueImages.map((img, idx) => (
                    <button 
                        ...
                        className={`flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-all ${
                            activeImage === img ? 'border-blue-600 opacity-100 ring-2 ring-blue-100' : 'border-transparent opacity-70 hover:opacity-100'
                        }`}
                    >
        ...
        <div className="w-full md:w-1/2 p-6 md:p-8 overflow-y-auto">
            ...
            <div className="sticky bottom-0 bg-white pt-4 border-t border-gray-100">
                 <button 
                    ...
                    className="w-full bg-blue-700 hover:bg-blue-800 text-white py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-3 shadow-lg hover:shadow-blue-200 transition-all active:scale-[0.98]"
                >
